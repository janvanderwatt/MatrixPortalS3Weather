...
*/
Adafruit_ImageReader::Adafruit_ImageReader(FS &fs) { filesys = &fs; }
...
ImageReturnCode Adafruit_ImageReader::drawBMP(const char *filename,
                                              ...
                                              int16_t y, boolean transact, uint8_t brightness) {
    ...
    return coreBMP(filename, &tft, tftbuf, x, y, NULL, transact, brightness);
}
...
ImageReturnCode Adafruit_ImageReader::loadBMP(const char *filename,
                                              Adafruit_Image &img, uint8_t brightness) {
    ...
    return coreBMP(filename, NULL, NULL, 0, 0, &img, false, brightness);
}
...
ImageReturnCode Adafruit_ImageReader::coreBMP(
    ...
    boolean transact,
    uint8_t brightness) { // SD & TFT sharing bus, use transactions
    ...
    uint16_t r, g, b;          // Current pixel color
    ...
    if (readLE16() == 0x4D42) { // BMP signature
        ...
        bmpWidth = readLE32();
        bmpHeight = readLE32();
        Serial.printf("[W:%d,H:%d]", bmpWidth, bmpHeight);
        ...
        Serial.printf("planes:[%d], compression:[%d], depth:[%d]", planes, compression, depth);
